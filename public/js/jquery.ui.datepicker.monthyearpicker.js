...
console.log("Successfully loaded jquery.ui.datepicker.monthyearpicker.js!");
(function($, undefined) {
    ...
    $.extend($.datepicker, {
        ...
        _generateMonthPickerHTML_MonthYearPicker: function(inst, minDate, maxDate, drawMonth, inMinYear, inMaxYear) {
            ...
            // Change Month Here [Xi En]
            let selected_year = $(".ui-datepicker-title .ui-datepicker-year").html();
            let tmp_month_arr = [];
            if (month_option_hashmap[selected_year])
                tmp_month_arr = month_option_hashmap[selected_year].map(a => month_txt_arr.indexOf(a));

            for (var month = 0; month < 12;) {
                unselectable = (inMinYear && month < minDate.getMonth()) || (inMaxYear && month > maxDate.getMonth());

                // Check to see if month is available or not
                unselectable = !tmp_month_arr.includes(month);
            ...
            switch (screen) {
                ...
                case 3:
                    ...
                    var _updateYearPicker_MYP = function(year) {
                        ...
                        // Change YEAR HERE [Xi En] 
                        let tmp_arr = [];
                        if (month_option_year_hashmap[year])
                            tmp_arr = month_option_year_hashmap[year];
                        for (var i = 1; i <= 12; i++) {
                            unselectable = (minYear !== 'undefined' && year < minYear) || (maxYear !== 'undefined' && year > maxYear);

                            // Check to see if Year is Available or not
                            if (!(i == 1 || i == 12))
                                unselectable = !tmp_arr.includes(year);
